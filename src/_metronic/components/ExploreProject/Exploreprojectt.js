...
const Exploreproject = () => {
  ...
  const GetAllExpProject = async () => {
    ...
    await ApiGet(`exploreProjects?page=${page}&limit=${countPerPage}&search`)
      .then((res) => {
  ...
  const AddExpProject = async (e) => {
    ...
    if (Validation()) {
      setLoading(true);
      await ApiPost(`exploreProjects`, formData)
        .then((res) => {
  ...
  const EditExpProject = async (e) => {
    ...
    await ApiPut(`exploreProjects/${idForEdit}`, formData)
      .then((res) => {
        if (res?.data?.status === 200) {
  ...
  const DeleteExpProject = async () => {
    setLoading(true);
    await ApiPut(`exploreProjects/delete/${idForEdit}`)
      .then((res) => {
        ...
        <div className="p-2 mb-2">
          <div className="row d-flex justify-content-between mb-4 pr-3">
            <div className="col d-flex justify-content-between">
              <h2 className="pl-3 pt-2">Project</h2>
            </div>
